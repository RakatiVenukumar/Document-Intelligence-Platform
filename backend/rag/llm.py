...
class ChatLLMClient:
    ...
    def generate(self, question, context, citations):
        if self.provider == "openai":
            return self._generate_openai(question, context, citations)

        return self._generate_lmstudio(question, context, citations)

    def _generate_lmstudio(self, question, context, citations):
        payload = self._build_payload(question, context, citations)
        request = Request(
    ...
    def _generate_openai(self, question, context, citations):
        payload = self._build_payload(question, context, citations)
    ...
    def _build_payload(self, question, context, citations):
        system_prompt = (
